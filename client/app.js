(function(){
  ...
  $('#joinBtn').addEventListener('click', ()=>{
    ...
    if(!socket){ socket=io(); socket.on('joined',({symbol:s,roomId:r,game:g})=>{ symbol=s; meta.textContent=`Joined ${g} room ${r} as ${s}`; fetchLB(); }); socket.on('room_update',d=>{ /*no-op*/ }); socket.on('game_state', st=>render(st)); socket.on('game_over',({winner})=>{ meta.textContent += ` | Winner: ${winner}`; fetchLB(); }); socket.on('leaderboard_update',rows=>renderLB(rows)); }
